...
describe("#getSummerJamCount", function(){
  it("should add up all the songs that hit #1 during the summer months", function(){
    expect(getSummerJamCount(songs)).to.equal(summerJamCount(songs));
  });
  it("should be a number", function(){
    expect(Number.isInteger(summerJamCount(songs))).to.equal(true);
  });
});

describe("#getTotalDurationInSeconds", function(){
  it("should add up all of the duration in seconds", function(){
    expect(getTotalDurationInSeconds(songs)).to.equal(getDurationsInSeconds(songs).reduce(function(acc,val){ return acc+ val}, 0));
  });
   it("should be a number", function(){
    expect(Number.isInteger(getTotalDurationInSeconds(songs))).to.equal(true);
  });
});

describe("#getSongCountByArtist", function(){
  it("should return the number of songs by an artist", function(){
    expect(getSongCountByArtist(songs)["Mariah Carey"]).to.equal(4);
    expect(getSongCountByArtist(songs)["OMI"]).to.equal(1);
    expect(getSongCountByArtist(songs)["Beyonce"]).to.equal(5);
  });
